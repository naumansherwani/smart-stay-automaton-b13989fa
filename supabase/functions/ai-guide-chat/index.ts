...
- **AI Insights**: One-click weekly founder insights generated by Lovable AI (Gemini 2.5 Flash). Analyzes MRR, churn, plan/industry mix, **refund reasons + churn reasons together** and returns 5 insights + 3 recommendations + a dedicated "Refund Diagnosis" section that answers "Why are users requesting refunds?" by correlating refund reasons with churn reasons.
...
serve(async (req) => {
  ...
  try {
    ...
    const decision = await pickTierAndCheck(req.headers.get("Authorization"), "chat", "ai-guide-chat");
