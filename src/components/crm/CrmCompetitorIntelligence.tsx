...
import { Loader2, Sparkles, Globe, TrendingUp, TrendingDown, Minus, Shield, Eye, Plus, X } from "lucide-react";
    ...
    try {
      const { data, error } = await supabase.functions.invoke("crm-ai-assistant", {
        body: {
          ...
          data: { industry, competitors: names },
        },
      });
        ...
        <CardContent className="space-y-2">
          {competitors.map((c, i) => (
            <div key={i} className="flex gap-2">
              <Input placeholder={`Competitor ${i + 1} name...`} value={c} onChange={e => updateCompetitor(i, e.target.value)} />
      ...
      {analysis && (
        <>
          ...
          {analysis.radar_data && analysis.radar_data.length > 0 && (
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-sm">Competitive Positioning</CardTitle></CardHeader>
              <CardContent>
