              ...
              <Card key={p.name} className={`relative flex flex-col ${p.popular ? "border-primary ring-2 ring-primary/20" : p.highlight ? "border-yellow-500 ring-2 ring-yellow-500/30" : ""}`}>
                ...
                <CardContent className="flex-1 flex flex-col">
                  <ul className="space-y-3 flex-1 mb-6">
                    {p.features.map((f) => (
                       <li key={f} className={`flex items-start gap-2 text-sm ${f.startsWith("⭐") ? "font-semibold text-violet-600 dark:text-violet-400 crm-feature-star" : ""}`}>
                         <Check className={`w-4 h-4 mt-0.5 shrink-0 ${f.startsWith("⭐") ? "text-violet-500" : "text-success"}`} />
